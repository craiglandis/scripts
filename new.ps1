<#
.SYNOPSIS
    Creates 1 VM each from the commonly used Marketplace images for Windows and Linux.
...
.PARAMETER singleWindowsVM
    Just create a single Windows VM

.PARAMETER singleLinuxVM
...
.EXAMPLE
    New-VMsFromCommonImages.ps1 -location westcentralus -userName craig -password $password

    Creates the VMs in westcentralus using the specified username and password.

    New-VMsFromCommonImages.ps1 -location westeurope -userName craig -password $password
...
param(
    ...
    [string]$vmName,
    [string]$vmSize = 'Standard_DS1_v2', #'Standard_A1_v2',
    [switch]$useManagedDisk = $true,
    [string]$location = 'westus2',
    [switch]$disableBginfoExtension,
    [switch]$useSingleResourceGroup = $true,
    [switch]$useSingleStorageAccount = $true,
    [switch]$singleWindowsVM,
    [switch]$singleLinuxVM,
    [switch]$linuxVMsOnly,
...
$ofs = ' '
$azureRMModuleVersion = (get-module -Name 'AzureRM' -ListAvailable)[0].Version.ToString()
$azureRMComputeModuleVersion = (get-module -Name 'AzureRM.Compute' -ListAvailable)[0].Version
$azureRMComputeModuleVersion = [string]$azureRMComputeModuleVersion
Show-Progress "AzureRM $azureRMModuleVersion"
Show-Progress "AzureRM.Compute $azureRMComputeModuleVersion"
...
if ($publisherName -and $offer -and $skus)
{
    ...
    {        
        show-progress "Use -singleLinuxVM or -singleWindowsVM when specifying pub/offer/sku since there is no programmatic way to know the OS type from just the pub/offer/sku"
        exit
    }
}
...
if (!$linuxVMsOnly)
{
    if (!$singleLinuxVM)
    {
        ...
        else 
        {
            ...
            #$windowsImages += (get-azurermvmimage -Location $location -PublisherName 'microsoft-hyperv' -Offer 'rs5_preview' -sku '2019-datacenter')[-1]
        }        
    }

    if (!$singleWindowsVM -and !$singleLinuxVM)
    {
        $windowsImages += (get-azurermvmimage -Location $location -PublisherName 'MicrosoftWindowsServer' -Offer 'WindowsServer' -Skus '2016-Datacenter-smalldisk')[-1]
        ...
        # Win10 RS3 has a kernel bug where it's very slow and usually unreachable using a 1-core VM size, so using Win10 RS2 image instead for now
        $windowsImages += (get-azurermvmimage -Location $location -PublisherName 'MicrosoftWindowsDesktop' -Offer 'Windows-10' -Skus 'RS4-Pro')[-1]
    }
...
$images | foreach {
    ...
    if ($vmName -and ($singleWindowsVM -or $singleLinuxVM))
    {
    ...
    {            
        if ($configureWinRM)
        {
            $vm = Set-AzureRmVMOperatingSystem -VM $vm -Windows -ComputerName $vmName -Credential $cred -ProvisionVMAgent -WinRMHttp -WinRMHttps -WinRMCertificateUrl $certURL            
